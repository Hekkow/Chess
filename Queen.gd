...
func get_moves(board, pos):
	var moves = []
	var directions = [Vector2i(1, 1), Vector2i(1, -1), Vector2i(-1, -1), Vector2i(-1, 1), Vector2i(0, 1), Vector2i(0, -1), Vector2i(1, 0), Vector2i(-1, 0)]
...
func _init(_color):
	self.color = _color
	image = load("res://pieces/queen-" + color + ".svg")
